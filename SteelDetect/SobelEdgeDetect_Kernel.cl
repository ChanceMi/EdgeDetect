...
void initTiledWork(struct MyWork* myWork)
{
    ...
    bool 	powerOf4 = ( (uint)log2((float)myWork->kernelCount) % 2) == 0;
    ...
    myWork->localWidth	= ceil( (float)myWork->width  / (float)(sqrt((float)myWork->kernelCount)) );
    myWork->localHeight	= ceil( (float)myWork->height / (float)(sqrt((float)myWork->kernelCount)) );

    myWork->x_start		= baseX + ( myWork->tid % (uint)(sqrt((float)myWork->kernelCount)) ) * myWork->localWidth;
    myWork->x_end		= baseX + min( myWork->x_start + myWork->localWidth , myWork->width);
    myWork->y_start		= baseY + ( myWork->tid / (uint)(sqrt((float)myWork->kernelCount)) ) * myWork->localHeight;
